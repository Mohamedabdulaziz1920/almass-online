...
import PaymentMethodsFormFields from './payment-methods-form-fields'

export default function PaymentMethodsFormWrapper({ setting }: { setting: ISettingInput }) {
  ...
  async function onSubmit(values: ISettingInput) {
    ...
    try {
      ...
      if (!res.success) {
        ...
        setSaveStatus('saved')
        toast({ title: '✅ تم الحفظ', description: 'تم حفظ إعدادات طرق الدفع بنجاح' })
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-gray-800/40 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
          ...
          <div className="p-6">
            <PaymentMethodsFormFields form={form} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 p-4 bg-gray-800/40 rounded-xl border border-white/10">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button type="submit" disabled={isSubmitting || !isDirty} className={`min-w-[140px] ${isDirty ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white' : 'bg-gray-700 text-gray-400'}`}>
