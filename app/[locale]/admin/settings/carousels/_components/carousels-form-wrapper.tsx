...
import CarouselsFormFields from './carousels-form-fields'

export default function CarouselsFormWrapper({ setting }: { setting: ISettingInput }) {
  ...
  async function onSubmit(values: ISettingInput) {
    ...
    try {
      ...
      if (!res.success) {
        ...
        setSaveStatus('saved')
        toast({ title: '✅ تم الحفظ', description: 'تم حفظ إعدادات السلايدر بنجاح' })
        ...
        <div className="flex items-center justify-between gap-4 p-4 bg-gray-800/40 rounded-xl border border-white/10">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              className={`min-w-[140px] ${isDirty ? 'bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-700 hover:to-rose-700 text-white' : 'bg-gray-700 text-gray-400 cursor-not-allowed'}`}
            >
